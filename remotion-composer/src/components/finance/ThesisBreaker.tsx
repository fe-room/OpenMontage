...
export const ThesisBreaker: React.FC<ThesisBreakerProps> = ({ thesis, conditions, theme, brand, canvasMode = "margin-note", headerTreatment = "compact", sourceTreatment = "compact", sourceLabel, sourceDate, period, sampleData }) => {
  const frame = useCurrentFrame();
  const normalized = conditions.slice(0, 4).map((condition) =>
    typeof condition === "string" ? { title: condition } : condition
  );
  return (
    <FinanceFrame eyebrow="DECISION / THESIS BREAKER" source={{sourceLabel, sourceDate, period, sampleData}} theme={theme} brand={brand} canvasMode={canvasMode} headerTreatment={headerTreatment} sourceTreatment={sourceTreatment}>
