...
export const ThesisBreaker: React.FC<ThesisBreakerProps> = ({ thesis, conditions, eyebrow = "核心判断", prompt = "接下来，怎么继续想？", theme, brand, canvasMode = "margin-note", headerTreatment = "compact", sourceTreatment = "compact", sourceLabel, sourceDate, period, sampleData, complianceText }) => {
  const frame = useCurrentFrame();
  const normalized = conditions.slice(0, 4).map((condition) =>
    typeof condition === "string" ? { title: condition } : condition
  );
  return (
    <FinanceFrame eyebrow={eyebrow} source={{sourceLabel, sourceDate, period, sampleData}} theme={theme} brand={brand} canvasMode={canvasMode} headerTreatment={headerTreatment} sourceTreatment={sourceTreatment} complianceText={complianceText}>
      <div style={{ position: "absolute", left: "7%", right: canvasMode === "margin-note" ? "39%" : "7%", top: "13%", bottom: "12%" }}>
        <div style={{ fontFamily: FINANCE_MONO, fontSize: 19, letterSpacing: "0.08em", color: FINANCE_COLORS.vermillion }}>{eyebrow}</div>
        <div style={{ fontSize: 24, marginTop: 10, color: FINANCE_COLORS.muted }}>{prompt}</div>
