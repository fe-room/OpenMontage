...
export const EvidenceCard: React.FC<EvidenceCardProps> = ({
  ...
  sourceLabel, sourceDate, period, sampleData, complianceText, initialReveal = false,
}) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const valueProgress = initialReveal ? 1 : reveal(frame, 8, 8 + fps * 0.7);
  const valueY = interpolate(valueProgress, [0, 1], [28, 0]);
    ...
    <FinanceFrame eyebrow={evidenceIndex || "EVIDENCE"} source={source} theme={theme} brand={brand} canvasMode={canvasMode} headerTreatment={headerTreatment} sourceTreatment={sourceTreatment} complianceText={complianceText}>
      <div style={{ position: "absolute", left: isDocument ? "13%" : "7%", right: marginNote ? "39%" : isDocument ? "9%" : "7%", top: headerTreatment === "none" ? "9%" : "14%", bottom: "13%", display: "grid", gridTemplateRows: density === "sparse" ? "auto 1fr auto" : "auto auto 1fr auto", alignContent: "start", gap: density === "dense" ? 28 : 42 }}>
        ...
        <div style={{ alignSelf: "center", padding: density === "dense" ? "20px 0" : "52px 0 40px" }}>
          <div style={{ fontFamily: FINANCE_MONO, fontSize: isTable ? 104 : density === "sparse" ? 176 : 154, lineHeight: 0.9, fontWeight: 700, letterSpacing: "-0.055em", opacity: initialReveal ? 1 : reveal(frame, 8, 24), translate: `0 ${valueY}px` }}>{primaryValue}</div>
          {interpretation && !marginNote && <div style={{ marginTop: 38, maxWidth: 760, fontSize: 31, lineHeight: 1.5, opacity: initialReveal ? 1 : reveal(frame, 22, 38) }}>{interpretation}</div>}
        </div>
        {supportingMetrics.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: isTable ? "1fr" : `repeat(${Math.min(supportingMetrics.length, 2)}, minmax(0, 1fr))`, gap: "0 34px", borderTop: `2px solid ${FINANCE_COLORS.line}`, alignSelf: "end" }}>
            {supportingMetrics.map((metric, index) => (
              <div key={`${metric.label}-${index}`} style={{ padding: "24px 8px 22px 0", borderBottom: `2px solid ${FINANCE_COLORS.line}`, opacity: initialReveal ? 1 : reveal(frame, 24 + index * 5, 39 + index * 5) }}>
