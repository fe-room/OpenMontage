...
For each final spoken script section, in order:
1. Extract the narration text
2. Read `script.voice_performance` and the section's `delivery_cues`
...
8. Generate using `tts_selector` with an explicit section path such as
   `projects/<project-id>/assets/audio/narration-<section-id>.mp3`. The configured/project provider preference
...
9. Record `source_section_id`, `synthesis_mode: "segmented"`, and the applied
   `voice_performance` metadata on each narration asset.
10. Verify each file exists and its duration matches the section's expected
    timing (±15%). Regenerate or revise timing when a section falls outside the
    tolerance; do not hide accumulated drift until compose.
...
```json
{
  ...
  "assets": [
    {
      ...
      "type": "audio",
      "subtype": "narration",
      "path": "assets/audio/narration-s1.mp3",
  ...
  "generation_summary": {
    ...
    "narration_synthesis_mode": "segmented",
    "images_generated": 8,
...
- [ ] Narration asset section IDs and order match the final script
